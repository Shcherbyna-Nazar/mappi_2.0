...
class NearbyUtils(private val apiKey: String) {
    ...
    private val placesApiService: PlacesApiService = Retrofit.Builder()
        .baseUrl("https://maps.googleapis.com/maps/api/place/")
        .addConverterFactory(GsonConverterFactory.create())
        .build()
        .create(PlacesApiService::class.java)

    suspend fun fetchNearbyRestaurants(location: Location, radius: Int = 5000): List<Restaurant> {
    ...
    private suspend fun fetchAllNearbyPlaces(
        ...
        } while (pageToken != null && results.size < 57)
                    ...
                    if (response.isSuccessful) {
                        response.body()?.let { nearbyResponse ->
                            val filteredRestaurants = nearbyResponse.results.mapNotNull { result ->
                                ...
                                if (distance <= radius) {
                                    val decimalFormat = "%.2f".format(distance / 1000)
                                    Restaurant(
                                        id = result.place_id,
                                        name = result.name,
    ...
    private fun inferCuisineType(name: String?): String {
        val cuisineKeywords = mapOf(
            "Pizza" to "Italian", "Sushi" to "Japanese", "Burger" to "American",
            "Grill" to "Barbecue", "Taco" to "Mexican", "Indian" to "Indian",
            "BBQ" to "Barbecue", "Pasta" to "Italian", "Seafood" to "Seafood",
            "Steak" to "Steakhouse", "Noodles" to "Asian", "Ramen" to "Japanese",
            "Kebab" to "Middle Eastern", "Falafel" to "Middle Eastern", "Deli" to "Delicatessen",
            "Brasserie" to "French", "Bistro" to "French", "Burrito" to "Mexican",
            "Bagel" to "Bakery", "Smoothie" to "Healthy", "Sandwich" to "American",
            "Tavern" to "Bar"
        )
